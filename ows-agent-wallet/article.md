# Agents Sign Transactions Without Touching Your Private Keys

> How OWS lets AI agents pay for services using scoped API keys — while your private keys stay encrypted and never leave the vault.
...
![OWS Architecture](./ows-agent-wallet.png)
...
- **Wallet Vault** (`~/.ows/wallets/`) — AES-256-GCM encrypted wallet files, one per wallet
- **API Key Store** (`~/.ows/keys/`) — scoped tokens with re-encrypted wallet secrets (HKDF)
- **Policy Engine** (`~/.ows/policies/`) — declarative rules + custom executable policies
- **Signing Core** (Rust FFI) — mlock'd memory, zeroize on drop, anti-ptrace
- **Interface Layer** — CLI (`ows`), Node.js SDK, Python SDK, MCP server
