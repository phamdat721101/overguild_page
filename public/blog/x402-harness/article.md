# Agents Pay & Browse: No API Keys Needed

> x402 + Browser Harness lets AI agents browse, pay, and consume any web service in 2 seconds — no accounts, no API keys, no humans.
...
![x402 Payment Flow](./x402-flow.svg)
...
![Browser Harness + x402 Architecture](./harness-architecture.svg)

### 🎬 See It In Action

Video demo: [harness-demo.mp4](/blog/harness-demo.mp4)
...
![Traditional API vs x402 + Browser Harness](./comparison.svg)
...
- `harness-architecture.svg` — Combined Browser Harness + x402 architecture diagram
- `x402-flow.svg` — x402 payment flow (Client → 402 → Pay → Content)
- `comparison.svg` — Traditional API vs x402 comparison table
